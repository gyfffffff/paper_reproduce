...
for stock in topstocks.index:
    ...
    s_date = pd.to_datetime('2015-01-01')  datetime.datetime.strptime(, '%Y-%m-%d').date()
    e_date = pd.to_datetime('2023-01-01')datetime.datetime.strptime(, '%Y-%m-%d').date()
    validdata = data[['DATES',
                      'SeqOpenPrice',
                      'SeqClosePrice',
                      'SeqHighPrice',
                      'SeqLowPrice',
                      'SeqTurnoverVolume']][data["DATES"]>s_date
                                            & data['DATES']<e_date]
    print(validdata)
    break
