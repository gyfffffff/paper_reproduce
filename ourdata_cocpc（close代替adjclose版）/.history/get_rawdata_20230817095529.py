...
for stock in stocklist:
    ...
    volumeconunt[stock] = data['SeqTurnoverVolume'][:2500].mean()
    # if len(volumeconunt)>10:
    #     break
volumeconunt_df = pd.DataFrame(list(volumeconunt.items()), columns=['stock', 'avg_volume'])
topstocks = volumeconunt_df.sort_values(by=['avg_volume'], ascending=False)
topstocks.to_csv('topstocks_orderby_avg_volume.csv', index=False)
