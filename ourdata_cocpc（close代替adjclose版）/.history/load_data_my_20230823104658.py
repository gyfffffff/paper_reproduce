# 问题1：market share 源码中是volume/1e7, 我们需要改吗
# 问题2：美股有adjclose，我们有什么对应的数据吗,这个数据好像挺重要的
# 问题3：rep-dim=108, feat-dim=7 ?
...
class DataLoad():
    def __init__(self):
        ...
        # 修改1
        self.train_start_date = '2013-01-01'
        self.train_end_date = '2021-01-01'
        self.valid_start_date = '2021-01-01'
        self.valid_end_date = '2022-01-01'
        self.test_start_date = '2022-01-01'
        self.test_end_date = '2022-12-30'
            ...
            for i, line in enumerate(movement_f):  # descend
                ...
                if t == main_target_date:
                    ...
                    if -0.005 <= float(main_mv_percent) < 0.0055:  # discard sample with low movement percent
                        return None
                    y = _get_mv_percents(data)  # 就是_get_mv_class(data)，返回0,1
...
# 修改3
                    raw_adj_close.append(raw_movement_data.loc[revised_raw_date==data[0], 'Close'].to_numpy()[0])
        ...
        prices_and_ts = {
            ...
            'y': y,    # 主日期是否上涨
            'ms': ms,  # 市场份额
            'main_mv_class': main_mv_class,  # 和y一样
            'mv_class': mv_class,  # 和ys一样
            'main_mv_percent': main_mv_percent,  # 主日期波动
            'mv_percents': mv_percents,  # 日期序列波动
            'main_ms_percent': main_market_share,  # 主日期市场份额
            'prices': prices,  # 日期序列高开低收
    ...
    def batch_gen(self, phase):
        ...
        while True:
            ...
            valid_time_line = self.get_valid_time_line(rd_time.isoformat())  # 从一个随机日期开始的20天
        ...
        if len(y_batch) == self.batch_size:
            ...
            print('batch gen none!')   # 原因是总是采集失败，200次里都没有32次成功，所以我调整到300次
            return None
    ...
    def gen_graph(self, data_dict):
        ...
        return features, labels, data_dict['ts']
    ...
    def _get_dataset(self, phase):
        ...
        if phase == 'train':
            ...
            with open('./data/kdd17_like/train_stock_200_batchs.pkl', 'wb') as train_f:
            ...
            while b < 50:
                ...
                #     print('wrong')
            with open('./data/kdd17_like/valid_stock_50_batchs.pkl', 'wb') as train_f:
                pickle.dump(train_batch_datasets, train_f)
            ...
            with open('./data/kdd17_like/test_stock_50_batchs.pkl', 'wb') as test_f:
