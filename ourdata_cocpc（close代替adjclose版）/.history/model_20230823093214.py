...
class CoCPC(nn.Module):  # 定义了一个名为CoCPC的类，该类继承自nn.Module，意味着它是一个PyTorch模型类。
    def __init__(self, timestep, batch_size, seq_len, rep_dim, feat_dim, var_num, dataset):

        super(CoCPC, self).__init__()  # 调用父类（nn.Module）的初始化方法，确保正确地初始化模型。
        # 输入的参数保存为模型的属性，以便后续在模型中使用。
        self.batch_size = batch_size
        self.seq_len = seq_len
        self.timestep = timestep
        self.rep_dim = rep_dim
        self.dataset = dataset
        # 定义了一个由线性层和激活函数组成的神经网络编码器。这个编码器用于将输入的特征（feat_dim维度）映射到一个较低维度的表示空间（rep_dim维度）。nn.Sequential表示将多个层按顺序堆叠成一个网络。
        self.encoder = nn.Sequential(
            nn.Linear(feat_dim, rep_dim), # 这是一个线性层（全连接层），输入特征的维度为feat_dim，输出特征的维度为rep_dim。
            # nn.BatchNorm1d(rep_dim),
            nn.ReLU()
        ).cuda()  # .cuda()用于将这个神经网络层移动到GPU上进行计算

        # 将线性层输出的标量值映射到0到1之间，用于产生注意力权重。
        self.gat_layer = nn.Sequential(
            nn.Linear(var_num, 1),# 输入特征的维度为var_num，输出特征的维度为1。该层的作用是将输入的特征映射到一个标量（单个数值）输出。
            nn.Sigmoid()
        )
        # 创建了一个GaussianCopula类的实例，用于建模高维数据的依赖关系。并将该实例移动到GPU（CUDA）上。
        self.g_cop = GaussianCopula(dim=var_num).cuda()
        # 一个具有rep_dim输入和rep_dim//2隐藏单元的单向GRU（门控循环单元）层。它用于对输入的表示进行序列建模。
        self.gru = nn.GRU(rep_dim, rep_dim//2, num_layers=1,
                          bidirectional=False, batch_first=True)
        # 定义了一个由多个线性层组成的列表。列表中的每个线性层将输入的维度rep_dim//2+64映射到rep_dim维度。列表长度为timestep，这意味着模型将根据序列长度timestep进行timestep次的映射操作。
        self.Wk  = nn.ModuleList([nn.Linear(rep_dim//2+64, rep_dim).cuda() for i in range(timestep)]) #256, 512
        # 定义了两个激活函数，用于进行概率计算。
        self.softmax  = nn.Softmax()
        self.lsoftmax = nn.LogSoftmax()
        # 这个预测器用于将输入的表示映射到输出空间，输出空间的维度为2。
        ...
        # 初始化神经网络模型中的权重和偏置，不同层采用不同的初始化方法
        def _weights_init(m):
            if isinstance(m, nn.Linear):  # 判断 m 是否是 nn.Linear 类型的层
                # kaiming_normal_ 初始化方法，该方法根据 ReLU 激活函数的特点来初始化权重。mode='fan_out' 表示将权重初始化为高斯分布，nonlinearity='relu' 表示该层使用 ReLU 激活函数。
                nn.init.kaiming_normal_(
                    m.weight, mode='fan_out', nonlinearity='relu')
            if isinstance(m, nn.Conv1d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, nn.BatchNorm1d):  # BatchNorm1d 是一种用于对输入数据进行批标准化的层。
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)

        # initialize gru
        for layer_p in self.gru._all_weights:  # 这是一个循环，遍历了GRU层的所有权重参数。self.gru._all_weights是GRU层内部的权重参数列表，包含了不同的权重和偏置。
            for p in layer_p:
                if 'weight' in p:  # 检查当前权重参数的名字中是否包含字符串'weight'。因为在GRU层中，参数名字中包含'weight'的参数是权重。
                    nn.init.kaiming_normal_(self.gru.__getattr__(p), mode='fan_out', nonlinearity='relu')

        # 用于调用之前定义的 _weights_init 函数，对模型中的其他神经网络层进行初始化
    ...
    def regenerate_data(self, x, t, y):
        '''
        通过随机选择其他样本的预测时期，并将其预测时期的信息替换为当前样本的预测时期，从而生成了负样本。
        generate positive and negative samples in each batch
        :param x: batchsize*seq_len*dim  # 输入数据，大小为 batchsize * seq_len * dim，其中 batchsize 表示批次大小，seq_len 表示序列长度，dim 表示输入特征的维度。
        ...
        pred_terms = t + 1
        # 创建一个大小为 batch_size 的数组 sentence_labels，并将其所有元素初始化为 1，即设定所有样本为正样本。
        ...
        for b in range(negative_num):
            ...
            # 数组用于表示整个批次中样本的索引，即numbers包含了从0到self.batch_size-1的连续整数。
            numbers = np.arange(0, self.batch_size)
            rand_num = np.random.choice(numbers[numbers!=b], 1) # 随机选择一个元素作为rand_num，但要求选取的元素不能等于当前负样本的索引b
            # 将第b个负样本的后pred_terms个时间步（即预测时期）的特征信息替换为另一个样本x[rand_num, -pred_terms:, :]的后pred_terms个时间步的特征信息。这样，第b个负样本就在预测时期上与其他样本有所不同。
            data[b, -pred_terms:, :] = x[rand_num, -pred_terms:, :]
            sentence_labels[b] = 0  # 将第b个负样本的标签设为0，表示这是一个负样本。正样本的标签在之前已经被设为1。

        #shuffle
        idxs = np.random.choice(self.batch_size, self.batch_size, replace=False)
        return data[idxs, ...], torch.from_numpy(sentence_labels[idxs]).float(), y[:, idxs, ...]

    def macro_context(self,  macro_loader, time_list, t_index):
        '''
    ...
    def forward(self, x, macro_loader, x_time_list, hidden, label):
        '''
        ...
        t_samples = random.randint(12, self.seq_len - self.timestep - 1)  #test for macro
        # input sequence is N*C*L, e.g. 8*1*20480
        # 生成1/4batchsize的负样本
        re_x, samples_label, label = self.regenerate_data(x, t_samples, label)
        re_x = re_x.cuda()
        label = label.cuda()
        ...
        c_q_t, loss_c, coef = self.macro_context( macro_loader, x_time_list, t_samples) #
        c_q_t = c_q_t.squeeze()
