...
class CoCPC(nn.Module):  # 定义了一个名为CoCPC的类，该类继承自nn.Module，意味着它是一个PyTorch模型类。
    ...
    def forward(self, x, macro_loader, adj_close_prices, x_time_list, hidden, label):
        ...
        # 生成1/4batchsize的负样本
        re_x, samples_label, label, adj_close_prices = self.regenerate_data(
            x, t_samples, label, adj_close_prices)
        re_x = re_x.cuda()
        label = label.cuda()
        ...
        c_q_t, loss_c, coef = self.macro_context(
            adj_close_prices[:t_samples+1, :], macro_loader, x_time_list, t_samples)
        c_q_t = c_q_t.squeeze()
