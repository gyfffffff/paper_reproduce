...
def train(args, model, train_loader, macro_loader, optimizer, epoch, batch_size):
    ...
    for idx, batch_idx in enumerate(perm):  # 前30batch里随机选一个batch
        data, label, time_list = train_loader[batch_idx]
        ...
        acc, loss, hidden, coef = model(data, macro_loader, time_list, hidden, label)
