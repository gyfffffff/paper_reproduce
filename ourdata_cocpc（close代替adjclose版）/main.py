...
if __name__ == '__main__':
    ...
    if args.dataset == 'acl18':
        ...
        train_loader = pickle.load(open('./data/kdd17_like/train_stock_200_batchs.pkl', 'rb'))
        validation_loader = pickle.load(open('./data/kdd17_like/valid_stock_50_batchs.pkl', 'rb'))
        test_loader = pickle.load(open('./data/kdd17_like/test_stock_50_batchs.pkl', 'rb'))
        args.feat_dim = 7
        macro_loader = macro_data_load(start_date_str='2013-01-01', end_date_str='2023-01-01', type=args.macro_type)

    args.vr_num = len(macro_loader) + 1  #add a stock
    # args.cpc_train = 'True'
    if args.cpc_train == 'True':
        CoCPC_main(logger, args, train_loader, validation_loader, macro_loader)  #just use small dataset for training Co-CPC
    ...
    checkpoint = torch.load(os.path.join(args.logging_dir,
                                         run_name + '-model_best.pth'))  # 加载检查点文件
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        valid_loss, valid_acc, valid_mcc, valid_mse, valid_mae = stock_validation(logger, args, cpc_model, clsf_model, test_loader)
    ...
    logger.info('***** best validation acc :{:.4f}\t mcc:{:.4f}\t mse:{:.4f}\t mae:{:.4f}\n'.format(valid_acc_list[best_index],
                                                                          valid_mcc_list[best_index],
                                                                          valid_mse_list[best_index],
                                                                          valid_mae_list[best_index]))
