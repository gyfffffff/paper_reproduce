...
def get_mse(pred, target):
    ...
    batchsize = pred.shape[1]
    pred_ = torch.argmax(pred, dim=2)
    return float(torch.sum(torch.square(pred_-target))/(T*batchsize))

def get_mae(pred, target):
    T = pred.shape[0]
    batchsize = pred.shape[1]
    pred_ = torch.argmax(pred, dim=2)
    return float(torch.sum(torch.abs(pred_-target))/(T*batchsize))
...
def stock_train(logger, args, cpc_model, stock_model, train_loader, epoch, optimizer):
    ...
    for idx, batch_idx in enumerate(perm):
        ...
        if idx % args.log_interval == 0:
            logger.info('Stock Train Epoch: {} [{}/{} ({:.0f}%)]\tacc:{:.4f}\tmcc:{:.4f}\tmse:{:.4f}\tmae:{:.4f}\tLoss: {:.6f}'.format(
                epoch, idx, len(train_loader),
                100. * idx / len(train_loader), acc, mcc, mse, mae, loss.item()))
    total_loss /= len(train_loader)
    logger.info(' ====> Stock Training Set: Average loss: {:.4f}\t Average acc: {:.4f}\t Var acc: {:.4f}\t Average mcc: {:.4f}\t Var mcc:{:.4f}\t Average mse: {:.4f}\t Average mae: {:.4f}\n'.format(
        total_loss, np.mean(acc_list), np.var(acc_list), np.mean(mcc_list), np.var(mcc_list), np.mean(mse_list), np.mean(mae_list)
    ))
    return total_loss
...
def stock_validation(logger, args, cpc_model, stock_model, valid_loader):
    ...
    with torch.no_grad():
        ...
        total_mse /= len(valid_loader)
        total_mae /= len(valid_loader)

    logger.info('===>Stock Validation set: Average loss: {:.4f}\tAccuracy:{:.4f}\t MCC:{:.4f}\n'.format(
        total_loss, total_acc, total_mcc, total_mse, total_mae))

    return total_loss, total_acc, total_mcc, total_mse, total_mae
