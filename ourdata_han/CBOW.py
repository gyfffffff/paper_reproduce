...
for epoch in trange(epoch):
    ...
    for context, target in tqdm(data):
        ...
        context_vector = make_context_vector(
            context, word_to_idx).to(device)  # 拿到上下文词的编号
        target = torch.tensor([word_to_idx[target]]).cuda()  # 拿到中心词的编号
