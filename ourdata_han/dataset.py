...
class SnP500Dataset:
    def __init__(self, flags):
        ...
        self.date_min, self.date_max, self.max_date_len, self.max_news_len, \
            self.wordvec = self.build_vocab(self.flags.tweet_dir)# 所有tweet中最早和最晚日期，单日最多推数，高频词词向量
        ...
        self.stock_dict, self.down_bound, self.up_bound = \
            self.load_stock_history()  # 股票每日波动，上下界使用stockNet
        ...
        self.num_words = 0
        self.date_tweets = self.load_tweets(self.flags.tweet_dir) # 每只股票每天推文词的idx
    ...
    def build_vocab(self, input_dir):
        ...
        word2vec = np.zeros((32000, 100))
        sample_vec = word2vec_dict['国际']
        word2vec[0,:] = np.zeros(len(sample_vec)) # <PAD>
        # word2vec.append(np.array([0.] * ))
        word2vec[1,:] = np.ones(len(sample_vec)) # <UNK>
        # word2vec.append(np.array([1.] * ))
        for w_idx in range(2, final_size):
            word2vec[w_idx, :] = word2vec_dict[self.idx2word[w_idx]]
            # word2vec.append(np.array())
        ...
        for filename in file_names:
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                for l in list(f):
                    ...
                    if not (date(2019, 1, 1) <= stock_date < date(2023, 1, 1)):
                        continue
    ...
    def map_stocks_tweets(self):
        ...
        for stock_name in self.stock_dict:
            ...
            for i in range(stock_days):
                ...
                if date(2019, 1, 1) <= label_date < date(2022, 5, 1):
                    train_x.append(ex)
                ...
                elif date(2022, 5, 1) <= label_date < date(2022, 9, 1):
                    dev_x.append(ex)
                    dev_y.append(label)
                ...
                elif date(2022, 9, 1) <= label_date < date(2023, 1, 1):
                    test_x.append(ex)
                    test_y.append(label)
