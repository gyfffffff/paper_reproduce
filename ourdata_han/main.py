...
run_name = 'v20230927'
logger = setup_logs("log", run_name)  # setup logs
    ...
    for step, ((days, day_lens, news_lens), labels) in enumerate(dataset):
        with tf.contrib.summary.record_summaries_every_n_global_steps(
            ...
            if log_interval and (step + 1) % log_interval == 0:
                ...
                logger.info('Epoch #%d\tStep #%d\tLoss: %.6f (%.1f steps/sec)' % (
                    ep + 1, step, loss_value, rate))
                start = time.time()

            if ep == 0 and step == 0:
                print('#trainable_params', get_num_trainable_params(model))
                logger.info('#trainable_params', get_num_trainable_params(model))
...
def test(model, dataset, class_weights, show_classification_report=False,
    ...
    logger.info('%s set: Average loss: %.6f, Accuracy: %.3f%% (%.3f sec)' %
          (ds_name, avg_loss.result(), 100 * accuracy.result(), end - start))
...
def run(flags_obj):
    ...
    logger.info('Using device %s, and data format %s.' % (device, data_format))
    print('Load dataset..', flags_obj.pickle_path)
    logger.info('Load dataset..', flags_obj.pickle_path)
    ...
    with tf.device(device):
        for ep in range(flags_obj.train_epochs):
            ...
            if dev_loss.numpy() < best_acc_ep[2]:
                ...
                logger.info('Save checkpoint', checkpoint_prefix)
            ...
            logger.info('Min loss {:.6f}, dev acc. {:.3f}%, ep {} \n'.format(
                best_acc_ep[2], best_acc_ep[0] * 100., best_acc_ep[1] + 1))
        latest_checkpoint = tf.train.latest_checkpoint(flags_obj.model_dir)
        print('Load the last checkpoint..', latest_checkpoint)
        logger.info('Load the last checkpoint..', latest_checkpoint)
