...
raw_data_path = r'D:\baiduyundownload\Data\Daily\2010-01-01,2023-07-05'
raw_data_path2 = r'data\temp_price'
preprocess_data_path = r'data\price\preprocessed'


def preprocess(file):
    with open(os.path.join(raw_data_path2, file), 'r') as f:
        data = pd.read_csv(f)
        date_time = data['Date']
    ...
    # print('shifted mv:', data['Adj Close'].shift(-1)[:3])
        ...
        with open(os.path.join(preprocess_data_path, filename), 'w') as wf:
...
invalidsock = []
stock_list = list(pd.read_csv('stock_list.csv', usecols=['ts_code'])['ts_code'].loc[:])
count = 0
for i in range(len(stock_list)):
    ...
    e_date = pd.to_datetime('2022-12-31')
    ...
    validdata.to_csv(os.path.join(raw_data_path2,filename), index=False)
    try:
        preprocess(filename)
        count += 1
        # if(count == 50):
        ...
        os.remove(os.path.join(raw_data_path2, filename))
        invalidsock.append(filename.split('.csv')[0])
        pass

import pickle
pickle.dump(invalidsock, open('data/invalidstock.pkl', 'wb'))
